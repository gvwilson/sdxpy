...
We constructed objects to create regular expressions in [%x re %].
...
<div class="table" id="parse-grammar" caption="Regular expression grammar." markdown="1">
| Meaning                   | Character |
| ------------------------- | --------- |
...
we will create a tree of objects ([%f parse-expression-tree %])
rather than instances of the regular expression classes from [%x re %];
the exercises will tackle the problem of converting the former to the latter.

[% figure
   slug="parse-expression-tree"
...
## Assembling the Tree {: #parse-tree}
...
and compress them later ([%f parse-mechanics %]).
