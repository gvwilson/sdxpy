...
def main():
    ...
    prose = {a: open(Path("src", a, "index.md"), "r").read() for a in slugs}
    ...
    print(
        f"{slugs[0]} <- {slugs[1]}: {', '.join(sorted(defined[slugs[0]].intersection(used[slugs[1]])))}"
    )
    print(
        f"{slugs[0]} -> {slugs[1]}: {', '.join(sorted(defined[slugs[1]].intersection(used[slugs[0]])))}"
    )
